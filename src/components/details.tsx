  ...
  return (
    <div>
        <div className="w-full h-[40px] mt-4 ml-6 flex justify-between items-center">
            <button onClick={onClose} className="flex gap-1 text-sm font-medium">
                <img src={close} className="" alt="close X" />
                <p>Close(Esc)</p>
            </button>
            <div className="flex w-5/12">
          <div className="flex items-center gap-1 w-5/12 text-sm font-medium" onClick={() => markAsReadUnread(email.id)}>
            <img src={unread} alt="open envelop logo" className="w-5 h-5"/>
            <label htmlFor="unreadLOgo">
              {read ? "Mark as unread" : "Mark as read"}
            </label>
          </div>
          <div className="flex gap-1 w-5/12 text-sm font-medium" onClick={() => makeArchiveUnarchive(email.id)}>
